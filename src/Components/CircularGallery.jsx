...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  // Split text into lines, for example at ":" or based on a max width
  let lines = [text];
  if (text.includes('\n')) {
    lines = text.split('\n');
  } else if (text.includes(': ')) {
    lines = text.split(': ').map((l, i, a) => i < a.length - 1 ? l + ':' : l);
  }
...
class Title {
  ...
  createMesh() {
    const { texture, width, height, linesCount } = createTextTexture(this.gl, this.text, this.font, this.textColor);
    ...
    const textHeight = this.plane.scale.y * 0.15 * (linesCount || 1);
